...
class Dll:
    ...
    def shuffle(self):
        if (self.head is None):
            raise Exception("There is not enough songs added to playlist in order to shuffle.")
        else:
            current = self.head
...
link.add('Hotel California', 'Eagles')
link.add('Bring Me to Life', 'Evanescence')
link.add('In the End', 'Linkin Park')
link.add('Chop Suey!', 'System of a Down')
link.add('Love Me Dead', 'Ludo')
link.add('Would You Love a Monster Man?', 'Lordi')
